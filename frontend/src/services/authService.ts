...
export class AuthService {
  ...
  static async login(credentials: LoginRequest): Promise<AuthResponse> {
    try {
      logger.info('🔐 Tentative de connexion pour:', credentials.email);
  ...
  static async register(userData: RegisterRequest): Promise<RegisterResponse> {
    try {
      logger.info('📝 Tentative d\'inscription pour:', userData.email);
  ...
  static async setupMFA(): Promise<MFASetupResponse> {
    try {
      logger.info('🔒 Configuration du MFA...');
  ...
  static async disableMFA(otpCode: string): Promise<void> {
    try {
      logger.info('🔓 Désactivation du MFA...');
  ...
  static async healthCheck(): Promise<boolean> {
    try {
      ...
      logger.error('❌ Service d\'authentification non disponible:', error);
      return false;
    }
  }
} 